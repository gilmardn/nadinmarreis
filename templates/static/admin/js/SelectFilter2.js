  ...
  window.SelectFilter = {
    init: function (field_id, field_name, is_stacked) {
      ...
      quickElement(
        ...
        'title',
        interpolate(gettext('Type into this box to filter down the list of available %s.'), [field_name])
      );
      ...
      quickElement(
        ...
        'help-tooltip search-label-icon',
        'title',
        interpolate(gettext('Type into this box to filter down the list of selected %s.'), [field_name])
      );
      ...
      const clear_all = quickElement(
        ...
        gettext('Remove all'),
        'title',
        interpolate(gettext('Click to remove all chosen %s at once.'), [field_name]),
        'href',
        '#',
        'id',
    ...
    refresh_filtered_warning: function (field_id) {
      ...
      warning.textContent = interpolate(
        ngettext('%s selected option not visible', '%s selected options not visible', count),
        [count]
      );
      if (count > 0) {
    ...
    filter_key_down: function (event, field_id, source, target) {
      ...
      if ((event.which && event.which === 40) || (event.keyCode && event.keyCode === 40)) {
        source_box.selectedIndex =
          source_box.length === source_box.selectedIndex + 1 ? 0 : source_box.selectedIndex + 1;
      }
      // up arrow -- wrap around
      if ((event.which && event.which === 38) || (event.keyCode && event.keyCode === 38)) {
        source_box.selectedIndex =
          source_box.selectedIndex === 0 ? source_box.length - 1 : source_box.selectedIndex - 1;
      }
    },
  };
